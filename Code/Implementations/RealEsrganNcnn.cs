    ...
    class RealEsrganNcnn : ImplementationBase
    {
        ...
        public static async Task Run(string inpath, string outpath, ModelData mdl)
        {
            ...
            await NcnnUtils.RunPerFolder(inpath, outpath, (inDir, outDir) =>
                $"{opt} cd /D {Path.Combine(Paths.binPath, Imps.realEsrganNcnn.dir).Wrap()} & {exeName} -i {inDir.Wrap()} -o {outDir.Wrap()}" +
                $" -g {gpu_ids} -m {NcnnUtils.currentNcnnModel.Wrap()} -n esrgan-x4 -s {scale} {tta} {ts}", OutputHandler);
