...
namespace CupscaleTests
{
    public class VideoEncoderTests
    {
        ...
        [Theory]
        [InlineData("H.264 (CPU)", "High", "-c:v libx264 -preset slow -crf 18")]
        [InlineData("H.264 (CPU)", "Lossless", "-c:v libx264 -preset veryslow -qp 0")]
        [InlineData("H.265 (CPU)", "Lossless", "-c:v libx265 -preset slow -x265-params lossless=1")]
        [InlineData("H.264 (NVENC)", "Lossless", "-c:v h264_nvenc -preset p7 -tune lossless")]
        [InlineData("AV1 (NVENC)", "Lossless", "-c:v av1_nvenc -preset p7 -rc vbr -cq 16 -b:v 0")]
        [InlineData("AV1 (NVENC)", "Normal", "-c:v av1_nvenc -preset p7 -rc vbr -cq 34 -b:v 0")]
        ...
        {
            Assert.Equal("-c:v libsvtav1 -preset 6 -svtav1-params lossless=1", VideoEncoders.GetArgs("AV1 (CPU)", "Lossless", "", full));
            Assert.Equal("-c:v libaom-av1 -cpu-used 6 -row-mt 1 -crf 26 -b:v 0", VideoEncoders.GetArgs("AV1 (CPU)", "High", "", essentials));
        }

        [Fact]
        public void Custom_PassesArgsThrough_AndUnknownFallsBackToH264()
        {
            Assert.Equal("-c:v libx264 -crf 12", VideoEncoders.GetArgs(VideoEncoders.Custom, "High", "  -c:v libx264 -crf 12 ", essentials));
            Assert.Equal("-c:v libx264 -preset slow -crf 18", VideoEncoders.GetArgs(VideoEncoders.Custom, "High", "", essentials));
